...
class EmojiSpannableStringBuilder : SpannableStringBuilder {
    ...
    private class WatcherWrapper internal constructor(val mObject: Any) : TextWatcher, SpanWatcher {
        ...
        override fun onSpanAdded(text: Spannable, what: Any, start: Int, end: Int) {
            LogUtils.tag("ATAT").d("onSpanAdded text:"+ text + ", class:"+text.javaClass)
        ...
        override fun onSpanRemoved(text: Spannable, what: Any, start: Int, end: Int) {
            LogUtils.tag("ATAT").d("onSpanRemoved text:"+ text + ", class:"+text.javaClass)
            if (mBlockCalls.get() > 0 && isImageSpan(what)) {
                return
            }
            val mentionUsers = text.getSpans(start, end, IntegratedBgSpan::class.java)
            if (mentionUsers != null && mentionUsers.isNotEmpty()) {
                for (user : IntegratedBgSpan in mentionUsers) {
                    LogUtils.tag("ATAT").d("MentionUser : $user")
                }
            }
